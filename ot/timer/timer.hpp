...
#include <ot/timer/mt-kahypar/libmtkahypar.h>
#include <climits>
#include <memory>
#include <cstdint>
#include <bitset>
...
//    tf::Executor _executor{1};
    ...
    // sorted (according to num_successors) prop_cands
    std::vector<Pin*> _bprop_cands_sorted; // for bpins, num_successors is pin->_fanin 
    ...
    // results from get_num_pin_perlevel_perpartition() 
    std::vector<std::vector<int>> _ftopo_partitioned_num_pins_level;
    std::vector<std::vector<int>> _btopo_partitioned_num_pins_level;
    ...
    // assign level number to each pin  
    void _assign_flevel_list();
    void _assign_blevel_list();
    ...
    // get number of pin in each level for each partitions
    void _get_num_pin_perlevel_perpartition();
