...
//
  ...
  for(int i=0; i<num_partition; i++) {
    ...
    if(one_partition_pins.size() != 0) { // this size may be zero??? still need to check
...
void Timer::_build_partitioned_taskflow() {
  ...
  for(auto& pin_vec : _btopo_partitioned_pins) {
    std::vector<int> num_pin_per_level = _btopo_partitioned_num_pins_level[index];
    btask[index] = _taskflow.emplace([this, pin_vec, num_pin_per_level] () mutable{
      int count_level = 1;
      int count_pin = 0;
      while(count_pin < pin_vec.size()) {
        Pin* p = pin_vec[count_pin];
        if(!p->_isbRep) {
          _bprop_rat(*p); // 1 typical task for a p to update timing in backward progragation
        }
        else {
          int cur_task_status = 0;
          if(p->_btask_status.compare_exchange_strong(cur_task_status, 1)) {
            _bprop_rat(*p); // 1 typical task for a p to update timing in backward progragation
            p->_btask_status.store(2, std::memory_order_release);
          }
          else { // if this task is being done or done
                 // i.e., p->_btask_status = 1 or 2
           
            if(p->_btask_status.load(std::memory_order_acquire) == 1) { // when this partition get a task that another thread is doing
              if(count_pin < num_pin_per_level[count_level] - 1) { // if there is any task left for this level, go to get that task
                count_pin ++;
                continue;
              }
              else { // if there is no task left for this level, this partition should wait for all the tasks of this level finish.
                for(size_t i=num_pin_per_level[count_level-1]; i<num_pin_per_level[count_level]; i++) { // make sure all the pins of this level has finished
                  if(pin_vec[i]->_isbRep) {
                    while(pin_vec[i]->_btask_status.load(std::memory_order_acquire) != 2) {}
                  }
                }
                count_pin ++;
                count_level ++;
...
//        std::cerr << "count_level = " << count_level << "\n";
        if(count_pin == num_pin_per_level[count_level] - 1) { // when this level pins finish
          for(size_t i=num_pin_per_level[count_level-1]; i<num_pin_per_level[count_level]; i++) { // make sure all the pins of this level has finished
            if(pin_vec[i]->_isbRep) {
              while(pin_vec[i]->_btask_status.load(std::memory_order_acquire) != 2) {}
            }
        ...
        count_pin ++; // this means either the partition finish a unrep task or this partition finish a rep task
      }
    });
    index++;
  }
